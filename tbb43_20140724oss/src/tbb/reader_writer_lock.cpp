...
namespace tbb
{
    ...
    ITT_SYNC_CREATE(this, _T("tbb::reader_writer_lock"), _T(""));
			...
			else
			{
				...
				if (I->status == waiting_nonblocking)
				{
					...
					{ // no more writers
						...
						{ // an incoming writer is in the process of being added
		...
		void reader_writer_lock::start_read(scoped_lock_read* I)
		{
			...
			if (!I->next)
			{ // first arriving reader in my group; set RFLAG, test writer flags
		...
		void reader_writer_lock::end_write(scoped_lock* I)
		{
			...
			{ // No more writers; clear writer flag, test reader interest flag
				__TBB_ASSERT(writer_head, NULL);
